...
class InferenceWrapperBase(object):
  ...
  def build_graph_from_config(self, model_config, checkpoint_path):
    ...
    # hack to load variables from provided checkpoint
    vars_dict = {}
    vars_list = tf.global_variables()
    var_names_list = [v.op.name for v in vars_list]
    for ckpt_var_name, _ in tf.contrib.framework.list_variables(checkpoint_path):
      var_name = ckpt_var_name
      if 'lstm' in ckpt_var_name:
        if 'biases' in ckpt_var_name:
          var_name = ckpt_var_name.replace('biases', 'bias')
        elif 'weights' in ckpt_var_name:
          var_name = ckpt_var_name.replace('weights', 'kernel')

      if var_name not in var_names_list:
        continue
      else:
         index = var_names_list.index(var_name)      

      vars_dict[ckpt_var_name] = vars_list[index]
